...
"""
Function to read csv file generated by the rad teams python script. 
This will take the python CSV file, remove extraneous characters, turn it into a matrix of 1's and 0's and orient it properly. 
The 1's in the matrix represent known obstructions or walls and the 0's represent empty space. 
Each index of the matrix represents a centimeter as interpreted by the vacuum robot.
"""
...
"""
Identifies points that are too close (i.e. within radius) of a wall. 
This is being used to thicken walls for the flood fill algorithm.
"""
...
#############################################################################
"""
Reads the lines of the simulation data file provided by the radiation team at path: 
`data_file_path::String` and returns a RadSim data struct.
"""
